...
Reward components:
    ...
    -20..+5   Timeout                 (proximity partial credit; +5 bonus if within 20% of target dist)
    - 0.1    Per-step time penalty
    +/-5.0    Distance progress       (approach/retreat, clipped ±5)
    +/-0.5    Heading alignment       (cos of heading_error × 0.5)
...
class DroneEnv(gym.Env):
    ...
    def __init__(self, DEVICE: str, env_id: str):
        ...
        self.MAX_STEPS   = 400    # ~40 s at 10 Hz
    ...
    def get_reward(self, obs: np.ndarray):
        ...
        Termination conditions are checked in priority order — first match wins.
        Step shaping is only applied on non-terminal steps.

        Distance progress reward is skipped on step 1 of each episode
        because pre_dist is None (no previous distance to compare against).
        From step 2 onward, progress = pre_dist - current_dist is used.

        Obstacle slots obs[15:23] are present but NOT used in reward here.
        When real sensor values arrive, uncomment the hook at the bottom
        and tune the penalty weight.
        """
        ...
        if self.pre_dist is not None:
            ...
            if dist_change >= 0:
                reward += float(np.clip(dist_change * 5.0, 0.0, 5.0))  # moving closer
            else:
                reward += float(np.clip(dist_change * 10.0, -10.0, 0.0))  # moving away — 2x penalty
        self.pre_dist = dist

        # 3. Heading alignment — facing the target is rewarded
        #    cos(0) = 1.0 (perfect),  cos(pi) = -1.0 (facing away)
        reward += float(np.cos(heading_error)) * 1

        # 4. Battery efficiency — penalise accumulated energy waste per step
        reward -= ep_mah * 0.001

        # After distance progress:
        reward -= dist * 0.005  # small but consistent pressure toward target
        # At dist=40m this is -0.2/step, at dist=5m it's -0.025/step
    ...
    def reset(self, seed=None, **kwargs):
        ...
        Start a fresh episode:
          1. Teleport drone to home via Lua SIM_ERES_RESET trigger.
          2. Randomise target position (>= 20 m from NED origin).
          3. Switch to GUIDED mode, arm, and takeoff to TAKEOFF_ALT.
          4. Snapshot mAh for per-episode energy tracking.

        Target is drawn uniformly from [-100, 100] m in both north and east,
        with the 20 m dead-zone around origin rejected to avoid trivial episodes.

        seed=None (SB3 default) produces different targets every episode.
        A fixed seed will always produce the same target — useful for eval runs.
        ...
        while True:
            self.TARGET_NORTH = int(rng.integers(-40, 41))
            self.TARGET_EAST  = int(rng.integers(-40, 41))
            # Reject targets inside the 20 m dead-zone around origin
            if abs(self.TARGET_NORTH) >= 10 or abs(self.TARGET_EAST) >= 10:
                break
